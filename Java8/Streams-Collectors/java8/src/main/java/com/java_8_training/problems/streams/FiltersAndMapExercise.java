...
public class FiltersAndMapExercise {
    ...
    public static List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter(integer -> integer % 2 == 0)
    ...
    public static List<String> findPersonByName(String name) {
        ...
        personName = input.filter((str -> str.equals(name)))
                .collect(toList());
        return personName;
    }
    ...
    public static List<Integer> filterNull() {
        ...
        evenNumbers = input.filter((Objects::nonNull))
                .collect(toList());
        return evenNumbers;
    }
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age) {
        ...
        persons = personList.stream().filter(person -> Objects.equals(person.getAge(), age) && person.getName().equals(name))
                .collect(toList());
        return persons;
    }
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(List::stream).collect(toList());
    ...
    public static int findTheYoungestPerson(List<Person> personList) {
        ...
        int min = 0;
        min = personList.stream().map(person -> person.getAge()).reduce(Integer.MAX_VALUE, ((a, b) -> a > b ? b : a));
//        min = personList.stream().collect(Collectors.minBy(Person::getAge));
        return min;
    }
    ...
    public static double findAgeAverage(List<Person> personList) {
        double sum = 0;
        //TODO #13
        sum = personList.stream().map(Person::getAge).mapToInt(Integer::intValue).sum();
    ...
    public static Optional<Integer> findSquareDivisibleBy3() {
        ...
        firstSquareDivisibleByThree = someNumbers.stream()
                .filter(integer -> {
                    ...
                    int x = (int) sqrt;
                    return (Math.pow(sqrt,2) == Math.pow(x,2)) && integer % 3 == 0;
                }).findFirst();
    ...
    public static int sumStringLength() {
        ...
        int sum = 0;
        //TODO #16
        sum = strings.stream().map(String::length).mapToInt(Integer::intValue).sum();
        return sum;
    }
    ...
    public static int sumSalary(List<Person> personList) {
        int sum = 0;
        //TODO #17
        sum = personList.stream().map(person -> person.getSalary()).mapToInt(Integer::intValue).sum();
        return sum;
    }
